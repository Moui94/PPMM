"""
backend/routes/operations.py — AG-API mit Vorgänger-Menge.
"""
...
from backend.constants import get_kapazitaet_optionen, KAPAZITAET_AG, MASCHINEN
...
def _op_detail(op) -> dict:
    return {
        ...
        "kapazitaet_optionen": get_kapazitaet_optionen(op["ag_nr"]),
        "ist_fraes_ag":   op["ag_nr"] in (1, 2, 3),
        "maschinen_liste": list(MASCHINEN.items()) if op["ag_nr"] in (1,2,3) else [],
    }
...
def get_op(op_id):
    ...
    detail = _op_detail(op)
    # Vorgänger-Menge für Rückmeldungs-Vorschlag
...
def update_op(op_id):
    ...
    return success(_op_detail(get_operation_by_id(conn, op_id)))
...
def post_feedback(op_id):
    ...
    try:
        fb_id = create_feedback(
            conn,
            op_id=op_id, order_id=op["order_id"], ag_nr=op["ag_nr"],
